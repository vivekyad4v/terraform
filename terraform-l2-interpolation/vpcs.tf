resource "aws_vpc" "myvpc" {
  ...
  tags = "${merge(map("Name", format("%s-%s-vpc", var.orgname, var.environ)), var.tags)}"
}
...
resource "aws_subnet" "private_subnet" {
  ...
  tags = "${merge(map("Name", format("%s-%s-pvt-%s", var.orgname, var.environ, element(var.azs, count.index))), var.tags)}"
}
